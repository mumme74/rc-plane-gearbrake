...
#define HAS_INTERRUPT(config)                               \
  (config->wakeupdatafreq != KXTJ3_1057_wakeup_datarate_OFF)
 ...
 */
uint8_t KXTJ3_1057GSelbits(bool lowpower, KXTJ3_1057_gselection_t gsel)
{
  if (!lowpower) {
    if (gsel == KXTJ3_1057_gselection_8G)
      return KXTJ3_1057_GSEL_8G_14bit;
    else if (gsel == KXTJ3_1057_gselection_16G)
      return KXTJ3_1057_GSEL_16G_14bit;
  }
  return gsel;
}
 ...
 */
void KXTJ3_1057BuildReg1(KXTJ3_1057Driver *devp,
                         uint8_t *cr, bool on)
{
  cr[0] = KXTJ3_1057_CTRL_REG1;
  ...
  if (HAS_INTERRUPT(devp->config))
    cr[1] |= KXTJ3_1057_CTRL_REG1_DRDYE | KXTJ3_1057_CTRL_REG1_WUFE;
}
 ...
 */
static msg_t KXTJ3_1057I2CReadRegister(I2CDriver *i2cp, uint8_t sad,
                                       uint8_t reg, uint8_t *rxbuf, size_t n) {

  uint8_t txbuf = reg | KXTJ3_1057_MS;
 ...
 * @return              the operation status.
 */
static msg_t KXTJ3_1057I2CWriteRegister(I2CDriver *i2cp, uint8_t sad,
...
static size_t acc_get_axes_number(void *ip) {
  ...
  return KXTJ3_1057_ACC_NUMBER_OF_AXES;
}
 ...
 */
 ...
 msg_t KXTJ3_1057AccelerometerReadRaw(KXTJ3_1057Driver *devp, int16_t axes[]) {
  ...
  msg = KXTJ3_1057I2CReadRegister(devp->config->i2cp, devp->sad,
                                  ...
                                  KXTJ3_1057_ACC_NUMBER_OF_AXES * 2);
...
static msg_t acc_read_cooked(void *ip, float axes[]) {
  KXTJ3_1057Driver* devp;
  uint32_t i;
  int32_t raw[KXTJ3_1057_ACC_NUMBER_OF_AXES];
  msg_t msg;
...
static msg_t acc_reset_sensivity(void *ip) {
  ...
  case KXTJ3_1057_gselection_4G:
    sensitivity = KXTJ3_1057_ACC_SENS_4G; break;
  case KXTJ3_1057_gselection_8G:
    sensitivity = KXTJ3_1057_ACC_SENS_8G; break;
  case KXTJ3_1057_gselection_16G:
 ...
 * @retval MSG_RESET    otherwise.
 */
static msg_t acc_set_full_scale(KXTJ3_1057Driver *devp,
                                KXTJ3_1057_gselection_t gsel) {
  float newfs, scale;
  uint8_t i, buff[2];
  msg_t msg;
  ...
  if(newfs != devp->accfullscale) {
    ...
    /* Updating register.*/
    KXTJ3_1057BuildReg1(devp, buff, true);
    msg = KXTJ3_1057I2CWriteRegister(devp->config->i2cp,
                                   devp->sad,
                                   buff, 1);
 ...
 * @retval MSG_RESET    otherwise.
 */
static msg_t acc_set_selftest(KXTJ3_1057Driver *devp,
                                bool activate)
{
  uint8_t cr[2];
  msg_t msg;
  ...
  // first stop device;
  KXTJ3_1057BuildReg1(devp, cr, false);
  ...
  KXTJ3_1057BuildReg1(devp, cr, true);
  msg = KXTJ3_1057I2CWriteRegister(devp->config->i2cp,
                                   devp->sad, cr, 1);
...
static const struct KXTJ3_1057VMT vmt_device = {
  (size_t)0,
#if KXTJ3_1057_USE_ADVANCED
  acc_set_full_scale,
  acc_set_selftest
#endif
};
 ...
 *
 * @param[out] devp     pointer to the @p KXTJ3_1057Driver object
 *
 * @init
 */
void KXTJ3_1057ObjectInit(KXTJ3_1057Driver *devp) {
  ...
  devp->accaxes = KXTJ3_1057_ACC_NUMBER_OF_AXES;

  devp->state = KXTJ3_1057_STOP;

  devp->sad = KXTJ3_1057_SLAVE_ADDR;

  devp->lowpower = false;
}

/**
 * @brief   Configures and activates KXTJ3_1057 Complex Driver peripheral.
 *
 * @param[in] devp      pointer to the @p KXTJ3_1057Driver object
 * @param[in] config    pointer to the @p KXTJ3_1057Config object
 *
 * @api
 */
void KXTJ3_1057Start(KXTJ3_1057Driver *devp, const KXTJ3_1057Config *config) {
  ...
  osalDbgAssert((devp->state == KXTJ3_1057_STOP) ||
                (devp->state == KXTJ3_1057_READY),
                "KXTJ3_1057Start(), invalid state");

  devp->config = config;
  if (config->highAddress)
    devp->sad = KXTJ3_1057_SLAVE_ADDR_HIGH;
  ...
  cr[0] = KXTJ3_1057_CTRL_REG2;

  /* Control register 2 configuration block.*/
  cr[1] = KXTJ3_1057_CTRL_REG2_OWUF(config->dataoutfreq);

  /* Interrupt control register 1*/
  cr[2] = HAS_INTERRUPT(config) ?
            KXTJ3_1057_INT_CTRL_REG1_IEN |
                KXTJ3_1057_INT_CTRL_REG1_IEA :
            0;

  /*  Interrupt control register 2.*/
  cr[3] = KXTJ3_1057_INT_CTRL_REG2_XWUE(config->motion_interrupt.x)
         |KXTJ3_1057_INT_CTRL_REG2_YWUE(config->motion_interrupt.y)
         |KXTJ3_1057_INT_CTRL_REG2_ZWUE(config->motion_interrupt.z);
  if (cr[3] > 0)
    cr[3] |= KXTJ3_1057_INT_CTRL_REG2_ULMODE; // pulse interruptpin
  ...
  KXTJ3_1057I2CWriteRegister(devp->config->i2cp, devp->sad, cr, 3);

  // DATA_CTRL register, not in line with previous registers
  cr[0] = KXTJ3_1057_DATA_CTRL_REG;
  cr[1] = KXTJ3_1057_DATA_CTRL_ODR(config->dataoutfreq);
  KXTJ3_1057I2CWriteRegister(devp->config->i2cp, devp->sad, cr, 1);

  if (config->wakeupthreshold > 0) {
    cr[0] = KXTJ3_1057_WAKEUP_THRESHOLD_H;
    uint16_t threshold = config->wakeupthreshold << 4;
    cr[1] = (threshold & 0xFF00) >> 8;
    cr[2] = (threshold & 0x00F0);
    KXTJ3_1057I2CWriteRegister(devp->config->i2cp, devp->sad, cr, 2);
  }
...
/**
 * @brief   Deactivates the KXTJ3_1057 Complex Driver peripheral.
 *
 * @param[in] devp       pointer to the @p KXTJ3_1057Driver object
 *
 * @api
...
void KXTJ3_1057Stop(KXTJ3_1057Driver *devp) {
  ...
  osalDbgAssert((devp->state == KXTJ3_1057_STOP) ||
                (devp->state == KXTJ3_1057_READY),
                "KXTJ3_1057Stop(), invalid state");

  if (devp->state == KXTJ3_1057_READY) {
#if KXTJ3_1057_SHARED_I2C
    i2cAcquireBus((devp)->config->i2cp);
    i2cStart((devp)->config->i2cp, (devp)->config->i2ccfg);
#endif /* KXTJ3_1057_SHARED_I2C */
    ...
    cr[0] = KXTJ3_1057_CTRL_REG1; // REG1 and REG2 are not adjasent
    cr[1] = 0; // CTRL_REG1
    KXTJ3_1057I2CWriteRegister(devp->config->i2cp, devp->sad, cr, 1);
    ...
    cr[3] = KXTJ3_1057_INT_CTRL_REG1_IEA; // INT_CTRL_REG2, reset value of device
    KXTJ3_1057I2CWriteRegister(devp->config->i2cp, devp->sad, cr, 3);
